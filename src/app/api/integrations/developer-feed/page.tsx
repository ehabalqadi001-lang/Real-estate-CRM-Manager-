            ...
            <tbody className="divide-y divide-gray-100">
              {rows?.map((row) => (
                <tr key={row.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="p-4">
                    {row.status === 'processed' ? <span className="flex items-center gap-1.5 text-green-600 bg-green-50 px-2 py-1 rounded-md w-fit text-xs font-bold"><CheckCircle2 className="size-4" /> معالج</span> :
                     row.status === 'failed' ? <span className="flex items-center gap-1.5 text-red-600 bg-red-50 px-2 py-1 rounded-md w-fit text-xs font-bold"><AlertCircle className="size-4" /> فشل</span> :
                     <span className="flex items-center gap-1.5 text-yellow-600 bg-yellow-50 px-2 py-1 rounded-md w-fit text-xs font-bold"><Clock className="size-4" /> قيد الانتظار</span>}
                  </td>
