...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: typeof Clock }> = {
  ...
  paid:     { label: 'مدفوعة',  color: 'text-[var(--fi-emerald)] bg-[var(--fi-soft)]',    icon: Banknote },
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      {stats && stats.approvedAmount > 0 && (
        <div className="bg-[var(--fi-soft)] border border-green-200 dark:border-green-800 rounded-xl p-5 flex items-center justify-between">
          <div>
            <p className="font-semibold text-green-800 dark:text-green-300">
              يمكنك طلب صرف {stats.approvedAmount.toLocaleString('ar-EG')} ج
            </p>
            <p className="text-xs text-[var(--fi-emerald)] dark:text-green-400 mt-1">
              لديك عمولات معتمدة وجاهزة للصرف
            </p>
          </div>
          <a
            href="/dashboard/commissions/payouts"
            className="px-4 py-2 bg-[var(--fi-emerald)] text-white text-sm font-medium rounded-lg hover:opacity-90 transition-colors"
          >
            طلب الصرف
          </a>
        </div>
