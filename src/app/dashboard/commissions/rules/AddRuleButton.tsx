...
export default function AddRuleButton() {
  ...
  return (
    <>
      ...
      >
        <Plus size={15} /> {t('إضافة قاعدة', 'Add Rule')}
      </button>

      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-4 sm:p-6" dir={dir}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-black text-slate-900">{t('قاعدة عمولة جديدة', 'New Commission Rule')}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="text-xs font-bold text-slate-600 block mb-1">{t('اسم القاعدة', 'Rule Name')}</label>
                <input name="name" required placeholder={t('مثال: عمولة وكيل 2.5%', 'e.g. Agent Commission 2.5%')} className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#00C27C]/30" />
              </div>

              <div>
                <label className="text-xs font-bold text-slate-600 block mb-1">{t('نوع المستفيد', 'Beneficiary Type')}</label>
                <select name="commission_type" className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#00C27C]/30">
                  ...
                  <option value="developer">{t('مطور', 'Developer')}</option>
                </select>
              </div>

              <div>
                <label className="text-xs font-bold text-slate-600 block mb-1">{t('اسم المشروع (اختياري — اتركه فارغاً لتطبيقه على الكل)', 'Project Name (optional — leave blank to apply to all)')}</label>
                <input name="project_name" placeholder={t('اسم المشروع', 'Project Name')} className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#00C27C]/30" />
              </div>
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={() => setUsePercentage(false)}
                  className={`flex-1 py-2 rounded-xl text-sm font-bold border transition-all ${!usePercentage ? 'bg-[#00C27C] text-white border-[#00C27C]' : 'border-slate-200 text-slate-500'}`}>
                  {t('مبلغ ثابت ج.م', 'Flat Amount EGP')}
                </button>
              </div>

              {usePercentage ? (
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">{t('النسبة المئوية', 'Percentage')}</label>
                ...
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">{t('المبلغ الثابت (ج.م)', 'Flat Amount (EGP)')}</label>
              ...
              <div className="flex gap-3 pt-2">
                <button type="submit" disabled={loading}
                  ...
                  {loading ? t('جاري الحفظ...', 'Saving...') : t('حفظ القاعدة', 'Save Rule')}
                </button>
