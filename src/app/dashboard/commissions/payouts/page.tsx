...
const STATUS_CONFIG = {
  draft:            { label: 'مسودة',          color: 'text-slate-600',   bg: 'bg-slate-50 border-slate-200' },
    ...
    <div className="p-6 space-y-5" dir="rtl">
      ...
      <div className="flex items-center justify-between bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-black text-slate-900">صرف العمولات</h1>
            <p className="text-xs text-slate-400">{total} دفعة · إدارة وموافقة وصرف العمولات</p>
          </div>
        </div>
        <CreatePayoutButton />
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            { label: 'إجمالي',        value: stats.total,    color: 'text-slate-700',   bg: 'bg-slate-50',   icon: AlertCircle },
          ].map(s => (
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        {payouts.length === 0 ? (
          <div className="p-16 text-center">
            ...
            <p className="font-bold text-slate-600">لا توجد دفعات بعد</p>
            <p className="text-sm text-slate-400 mt-1">اعتمد العمولات أولاً ثم أنشئ دفعة صرف</p>
          </div>
        ) : (
          <div className="divide-y divide-slate-50">
            {payouts.map((payout: any) => {
              ...
              return (
                <div key={payout.id} className="p-4 hover:bg-slate-50/60 transition-colors flex items-center gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="font-bold text-slate-900 text-sm truncate">{payout.title}</p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Link
                      ...
                      className="inline-flex items-center gap-1 text-[11px] font-bold text-slate-600 hover:text-blue-600 bg-slate-100 hover:bg-blue-50 px-2.5 py-1.5 rounded-lg transition-colors"
                    >
