  ...
  return (
    <div className="bg-[var(--fi-paper)] border border-[var(--fi-line)] rounded-2xl overflow-hidden" dir="rtl">
      <div className="flex items-center gap-3 border-b border-[var(--fi-line)] p-4">
        ...
        <div>
          <h2 className="font-bold text-[var(--fi-ink)]">{t('مطالبة بيع جديدة', 'New Sale Claim')}</h2>
          <p className="text-xs text-[var(--fi-muted)]">{t('رفع عملية بيع للمراجعة والاعتماد', 'Submit a sale for review and approval')}</p>
        </div>
      </div>
      ...
      <form action={action} className="grid gap-4 p-4 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2">
          <p className="text-xs font-black text-[var(--fi-muted)] uppercase tracking-widest mb-3 pb-2 border-b border-[var(--fi-line)]">
            {t('بيانات المشتري', 'Buyer Info')}
          </p>
        </div>

        <label className="block">
          <span className="mb-1.5 block text-xs font-black text-[var(--fi-muted)]">{t('اسم المشتري *', 'Buyer Name *')}</span>
          <input name="buyerName" required placeholder={t('الاسم الكامل', 'Full name')} className={inputClass} />
        </label>
        ...
        <label className="block">
          <span className="mb-1.5 block text-xs font-black text-[var(--fi-muted)]">{t('البريد الإلكتروني', 'Email')}</span>
          <input name="buyerEmail" type="email" placeholder="buyer@example.com" className={inputClass} />
        </label>

        <label className="block">
          <span className="mb-1.5 block text-xs font-black text-[var(--fi-muted)]">{t('الرقم القومي', 'National ID')}</span>
          <input name="buyerNationalId" placeholder={t('14 رقماً', '14 digits')} maxLength={14} className={inputClass} />
        </label>

        {/* Unit & financial */}
        <div className="sm:col-span-2 mt-1">
          <p className="text-xs font-black text-[var(--fi-muted)] uppercase tracking-widest mb-3 pb-2 border-b border-[var(--fi-line)]">
            {t('تفاصيل البيع', 'Sale Details')}
          </p>
        </div>
        ...
        {units.length > 0 && (
          <label className="sm:col-span-2 block">
            ...
            <select name="unitId" className={inputClass}>
              <option value="">{t('— اختر وحدة (اختياري) —', '— Select unit (optional) —')}</option>
              {units.map(u => (
        ...
        <label className="block">
          <span className="mb-1.5 block text-xs font-black text-[var(--fi-muted)]">{t('سعر البيع (ج.م) *', 'Sale Price (EGP) *')}</span>
          <input name="salePrice" type="number" min="1" required placeholder="0" className={inputClass} />
        </label>

        <label className="block">
          <span className="mb-1.5 block text-xs font-black text-[var(--fi-muted)]">{t('الدفعة المقدمة (ج.م)', 'Down Payment (EGP)')}</span>
          <input name="downPayment" type="number" min="0" placeholder={t('اختياري', 'optional')} className={inputClass} />
        </label>

        <label className="block">
          <span className="mb-1.5 block text-xs font-black text-[var(--fi-muted)]">{t('نسبة العمولة (%)', 'Commission Rate (%)')}</span>
          <input name="commissionRate" type="number" min="0" max="100" step="0.1" placeholder="2.5" className={inputClass} />
        </label>

        <label className="block">
          <span className="mb-1.5 block text-xs font-black text-[var(--fi-muted)]">{t('سنوات التقسيط', 'Installment Years')}</span>
          <input name="installmentYears" type="number" min="0" max="30" placeholder={t('0 = نقداً', '0 = Cash')} className={inputClass} />
        </label>

        <label className="block">
          <span className="mb-1.5 block text-xs font-black text-[var(--fi-muted)]">{t('تاريخ التعاقد', 'Contract Date')}</span>
        ...
        <label className="sm:col-span-2 block">
          ...
          <textarea
            name="notes"
            placeholder={t('أي تفاصيل إضافية عن الصفقة...', 'Any additional deal details...')}
            rows={2}
        ...
        <div className="sm:col-span-2">
          ...
          >
            <Plus size={16} />
            {pending ? t('جاري الرفع...', 'Submitting...') : t('رفع مطالبة البيع', 'Submit Sale Claim')}
          </button>
        </div>
      </form>
    </div>
...
export function MarkPaidButton({ claimId }: { claimId: string }) {
  ...
  return (
    <form action={action} className="inline">
      ...
      >
        <Banknote size={12} />
        {pending ? '...' : t('صرف العمولة', 'Pay Commission')}
      </button>
    </form>
  )
}
