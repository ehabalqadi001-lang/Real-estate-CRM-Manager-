  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-sm font-black text-[#0F8F83]">NEXUS Reports</p>
          <h1 className="mt-1 text-xl sm:text-3xl font-black text-[#102033] dark:text-white">تقارير العملاء</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {[
          { icon: <FileText className="size-5" />, value: draftCount,     label: 'مسودة',  color: 'text-slate-500' },
          { icon: <Clock className="size-5" />,    value: scheduledCount, label: 'مجدول',  color: 'text-[#C9964A]' },
          { icon: <CheckCheck className="size-5" />, value: sentCount,    label: 'أُرسل',  color: 'text-[#0F8F83]' },
        ].map((k) => (
      ...
      {reports.length === 0 ? (
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-[#DDE6E4] py-16 text-center">
          <Send className="size-10 text-slate-300" />
          <p className="font-bold text-slate-400">لا توجد تقارير بعد</p>
          <p className="text-xs text-slate-400">أنشئ أول تقرير لإرساله للعملاء</p>
        </div>
      ) : (
        <div className="space-y-3">
          {reports.map((r) => (
            <div key={r.id} className="flex items-start gap-3 rounded-2xl border border-[#DDE6E4] bg-white p-4 shadow-sm dark:bg-slate-900">
              <div className="mt-0.5 rounded-lg bg-[#0F8F83]/10 p-2 text-[#0F8F83]">
                <FileText className="size-4" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <p className="font-black text-[#102033] dark:text-white">{r.title}</p>
                ...
                <div className="mt-1 flex flex-wrap gap-3 text-xs font-semibold text-slate-500">
                  ...
                  {r.sent_at && (
                    <span className="text-[#0F8F83]">أُرسل: {new Date(r.sent_at).toLocaleString('ar-EG')}</span>
                  )}
                </div>
              </div>
