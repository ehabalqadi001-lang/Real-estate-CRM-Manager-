...
export default async function UnitDetailPage({ params }: PageProps) {
  ...
  const [unit, media, reservation, { t, numLocale }] = await Promise.all([
  ...
  const STATUS_CFG: Record<string, { label: string; color: string }> = {
    available: { label: t('متاح للبيع', 'Available'), color: 'text-emerald-700 bg-emerald-50 border-emerald-200 dark:bg-emerald-900/20 dark:border-emerald-700' },
    reserved:  { label: t('محجوز', 'Reserved'),       color: 'text-amber-700 bg-amber-50 border-amber-200 dark:bg-amber-900/20 dark:border-amber-700' },
    sold:      { label: t('مُباع', 'Sold'),            color: 'text-slate-500 bg-slate-100 border-slate-200 dark:bg-slate-800 dark:border-slate-700' },
  }
  ...
  const FINISHING_LABELS: Record<string, string> = {
    fully_finished: t('مشطّب بالكامل', 'Fully Finished'),
    semi_finished:  t('نص تشطيب', 'Semi Finished'),
    core_and_shell: t('بدون تشطيب', 'Core & Shell'),
    ultra_luxury:   t('تشطيب فندقي', 'Ultra Luxury'),
  }
  ...
  const specs = [
    { icon: Maximize2, label: t('المساحة', 'Area'),         value: unit.area_sqm ? `${unit.area_sqm} m²` : null },
    { icon: BedDouble, label: t('غرف النوم', 'Bedrooms'),    value: unit.bedrooms != null ? `${unit.bedrooms} ${t('غرف', 'br')}` : null },
    { icon: Bath,      label: t('الحمامات', 'Bathrooms'),    value: unit.bathrooms != null ? `${unit.bathrooms}` : null },
    { icon: Layers,    label: t('الدور', 'Floor'),           value: unit.floor != null ? `${t('الدور', 'Floor')} ${unit.floor}` : null },
    { icon: Home,      label: t('نوع الوحدة', 'Unit Type'),  value: TYPE_LABELS[unit.unit_type ?? ''] ?? unit.unit_type },
    { icon: Tag,       label: t('التشطيب', 'Finishing'),     value: FINISHING_LABELS[unit.finishing_type ?? ''] ?? unit.finishing },
    { icon: MapPin,    label: t('الاتجاه', 'Orientation'),   value: unit.orientation ?? null },
    { icon: Calendar,  label: t('تاريخ التسليم', 'Delivery Date'), value: unit.delivery_date ?? null },
  ].filter(s => s.value)
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5 max-w-4xl" dir="rtl">
      ...
      >
        <ArrowRight size={14} /> {t('العودة للوحدات', 'Back to Units')}
      </Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-5">
            ...
            {primaryImage ? (
              <div className="relative">
                <Image
                  ...
                  alt={primaryImage.title ?? unit.unit_number ?? t('صورة الوحدة', 'Unit Image')}
                  width={1200}
                  height={640}
              ...
              <div className="relative h-48 bg-gray-100 dark:bg-gray-800 flex flex-col items-center justify-center gap-2">
                ...
                <span className="text-xs text-gray-400">{t('لا توجد صور مرفوعة', 'No images uploaded')}</span>
                <div className="absolute top-3 end-3">
                  <span className={`text-sm font-bold px-3 py-1 rounded-full border ${cfg.color}`}>
                    {cfg.label}
                  </span>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-slate-100 dark:border-gray-800 p-5">
            ...
            <div className="mt-4">
              <p className="text-3xl font-black text-blue-700 dark:text-blue-400">
                {fmt(unit.price ?? 0)} {t('ج.م', 'EGP')}
              </p>
              {unit.area_sqm && unit.price && (
                <p className="text-sm text-slate-400 mt-1">
                  {fmt(Math.round(unit.price / unit.area_sqm))} {t('ج.م', 'EGP')} / m²
                </p>
              )}
            </div>
            ...
            {project?.down_payment_pct && (
              <div className="mt-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl p-3 flex items-center gap-2">
                ...
                <span className="text-sm text-blue-700 dark:text-blue-300 font-medium">
                  {t('مقدّم', 'Down')} {project.down_payment_pct}% + {t('تقسيط', 'Installments')} {project.payment_years} {t('سنوات', 'yrs')}
                </span>
              </div>
            )}
          </div>
          ...
          {specs.length > 0 && (
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-slate-100 dark:border-gray-800 p-5">
              <h2 className="font-black text-slate-800 dark:text-white mb-4">{t('المواصفات', 'Specifications')}</h2>
          ...
          {floorPlans.length > 0 && (
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-slate-100 dark:border-gray-800 p-5">
              <h2 className="font-black text-slate-800 dark:text-white mb-3">{t('المخططات', 'Floor Plans')}</h2>
              <div className="grid grid-cols-1 xs:grid-cols-2 gap-3">
                {floorPlans.map(fp => (
                  <a key={fp.id} href={fp.url} target="_blank" rel="noreferrer"
                    ...
                    <Image src={fp.thumbnail_url ?? fp.url} alt={fp.title ?? t('مخطط', 'Floor Plan')} width={400} height={128} className="w-full h-32 object-cover" />
                    <div className="p-2 text-xs text-gray-600 dark:text-gray-400">{fp.title ?? t('مخطط الطابق', 'Floor Plan')}</div>
                  </a>
                ))}
              </div>
            </div>
          ...
          {unit.description && (
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-slate-100 dark:border-gray-800 p-5">
              <h2 className="font-black text-slate-800 dark:text-white mb-2">{t('وصف الوحدة', 'Unit Description')}</h2>
        ...
        <div className="space-y-4">
          ...
          {reservation && (
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-semibold text-sm text-amber-800 dark:text-amber-300">{t('محجوزة', 'Reserved')}</span>
              </div>
              <p className="text-sm text-amber-700 dark:text-amber-400">
                {t('محجوزة لـ:', 'Reserved for:')} <strong>{reservation.client_name}</strong>
              </p>
              <p className="text-xs text-amber-600 mt-1">
                {t('تنتهي:', 'Expires:')} {new Date(reservation.expires_at).toLocaleString(numLocale)}
              </p>
            </div>
          )}
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-slate-100 dark:border-gray-800 p-4 space-y-3">
            <h3 className="font-bold text-slate-800 dark:text-white text-sm">{t('الإجراءات', 'Actions')}</h3>
            ...
            >
              <Building2 size={15} />
              {t('مقارنة مع وحدات أخرى', 'Compare with Other Units')}
            </Link>
          </div>
          ...
          {project?.commission_pct && (
            <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="font-semibold text-sm text-green-800 dark:text-green-300">{t('عمولة الوسيط', 'Broker Commission')}</span>
              </div>
              <p className="text-2xl font-black text-green-700 dark:text-green-400">{project.commission_pct}%</p>
              {unit.price && (
                <p className="text-xs text-green-600 mt-1">
                  ≈ {fmt(Math.round(unit.price * (project.commission_pct / 100)))} {t('ج.م', 'EGP')}
                </p>
              )}
            </div>
          )}
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-slate-100 dark:border-gray-800 p-4">
            <h3 className="font-bold text-slate-800 dark:text-white text-sm mb-3">{t('معلومات سريعة', 'Quick Info')}</h3>
            <dl className="space-y-2 text-sm">
              {[
                { label: t('المشروع', 'Project'),    value: project?.name ?? '—' },
                { label: t('المطوّر', 'Developer'),   value: developer?.name ?? project?.developer_name ?? '—' },
                { label: t('المنطقة', 'Area'),        value: project?.location ?? '—' },
                { label: t('الحالة', 'Status'),       value: cfg.label },
                { label: t('رقم الوحدة', 'Unit No.'), value: unit.unit_number ?? id.slice(0, 8) },
              ].map(({ label, value }) => (
