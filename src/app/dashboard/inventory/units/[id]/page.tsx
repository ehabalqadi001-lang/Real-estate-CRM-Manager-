...
import { Building2, ArrowRight, MapPin, Layers, BedDouble, Maximize2 } from 'lucide-react'
import { getUnit } from '@/domains/inventory/actions'
...
export default async function UnitDetailPage({ params }: PageProps) {
  ...
  const cfg = STATUS_CFG[(unit as any).status] ?? STATUS_CFG.available
  const project = (unit as any).projects
  const developer = project?.developers

  const specs = [
    { icon: BedDouble,  label: 'غرف النوم',   value: (unit as any).bedrooms ? `${(unit as any).bedrooms} غرف` : null },
    { icon: Maximize2,  label: 'المساحة',      value: (unit as any).area_sqm ? `${(unit as any).area_sqm} م²` : null },
    { icon: Layers,     label: 'الدور',        value: (unit as any).floor ? `الدور ${(unit as any).floor}` : null },
    { icon: Building2,  label: 'النوع',        value: TYPE_LABELS[(unit as any).unit_type ?? ''] ?? (unit as any).unit_type },
  ].filter(s => s.value)
    ...
    <div className="p-6 space-y-5 max-w-3xl" dir="rtl">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h1 className="text-2xl font-black text-slate-900">{(unit as any).unit_number ?? 'وحدة'}</h1>
            {project && (
        ...
        <p className="text-3xl font-black text-blue-700">{fmtFull((unit as any).price ?? 0)} ج.م</p>
        {(unit as any).price_per_sqm && (
          <p className="text-xs text-slate-400 mt-1">{fmtFull((unit as any).price_per_sqm)} ج.م / م²</p>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-5 space-y-3">
        ...
        <dl className="divide-y divide-slate-50">
          {[
            { label: 'التشطيب',         value: (unit as any).finishing },
            { label: 'الاتجاه',          value: (unit as any).orientation },
            { label: 'نسبة العمولة',     value: project?.commission_pct ? `${project.commission_pct}%` : null },
            { label: 'تاريخ التسليم',    value: (unit as any).delivery_date },
            { label: 'معرّف الوحدة',     value: (unit as any).id },
      ...
      {(unit as any).description && (
        <div className="bg-white rounded-2xl border border-slate-100 p-5">
          ...
          <p className="text-sm text-slate-600 leading-relaxed">{(unit as any).description}</p>
        </div>
      )}

      {/* Actions */}
      {(unit as any).status === 'available' && (
        <div className="bg-white rounded-2xl border border-slate-100 p-5">
