    ...
    return (
      <div className="flex h-[80vh] flex-col items-center justify-center p-4 sm:p-8 text-center" dir="rtl">
        <div className="bg-white p-4 sm:p-8 rounded-3xl border border-slate-200 max-w-lg w-full shadow-lg">
          ...
          <h2 className="text-2xl font-black text-slate-900 mb-3">{t('عذراً، لا يمكن الوصول لسجل هذا العميل', 'Sorry, cannot access this client record')}</h2>
          <p className="text-slate-600 mb-6 font-medium leading-relaxed">
            {t('قد يكون العميل غير موجود في قاعدة البيانات، أو أن الجدار الأمني (RLS) يمنع حسابك من عرض بيانات تخص شركة أخرى.', 'The client may not exist or the security policy (RLS) is preventing your account from viewing data belonging to another company.')}
          </p>

          <div className="bg-slate-50 border border-slate-100 p-3 rounded-xl mb-8 text-right">
            <p className="text-[10px] font-bold text-slate-400 mb-1">{t('التقرير التشخيصي (للمدير التقني):', 'Diagnostic report (for tech admin):')} </p>
          ...
          >
            {t('العودة لمسار المبيعات', 'Back to Sales Pipeline')} <ArrowRight size={18} />
          </Link>
        </div>
      </div>
    )
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8 min-h-screen bg-slate-50/50" dir="rtl">
      ...
      <div className="flex justify-between items-center">
        <Link href="/dashboard/leads" className="inline-flex items-center gap-2 text-slate-500 hover:text-blue-600 font-bold transition-colors bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-200">
          <ArrowRight size={18} /> {t('مسار المبيعات', 'Sales Pipeline')}
        </Link>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 bg-white">
          <div className="space-y-5 p-5 bg-slate-50 rounded-xl border border-slate-100">
            <h3 className="text-sm font-black text-slate-800 border-b border-slate-200 pb-2 mb-4">{t('بيانات التواصل', 'Contact Details')}</h3>
            <div className="flex items-center gap-4 text-slate-700">
              <div className="bg-white p-2.5 rounded-xl shadow-sm border border-slate-100 text-blue-600"><Phone size={18}/></div>
              <div className="flex-1">
                <p className="text-[10px] font-bold text-slate-400">{t('رقم الهاتف الأساسي', 'Primary Phone')}</p>
                <p className="font-bold text-md" dir="ltr">{lead.phone || t('غير مسجل', 'Not registered')}</p>
              </div>
              {lead.phone && (
                <SendWhatsAppButton
                  leadId={leadId}
                  phone={lead.phone}
                  leadName={lead.client_name || lead.full_name || t('عميل', 'Client')}
                />
              )}
            </div>
            <div className="flex items-center gap-4 text-slate-700">
              ...
              <div>
                <p className="text-[10px] font-bold text-slate-400">{t('البريد الإلكتروني', 'Email')}</p>
                <p className="font-bold text-md">{lead.email || t('غير مسجل', 'Not registered')}</p>
              </div>
            </div>
          </div>

          <div className="space-y-5 p-5 bg-slate-50 rounded-xl border border-slate-100">
            <h3 className="text-sm font-black text-slate-800 border-b border-slate-200 pb-2 mb-4">{t('الهدف الاستثماري', 'Investment Goal')}</h3>
            <div className="flex items-center gap-4 text-slate-700">
              <div className="bg-white p-2.5 rounded-xl shadow-sm border border-slate-100 text-emerald-600"><Building2 size={18}/></div>
              <div>
                <p className="text-[10px] font-bold text-slate-400">{t('نوع العقار المطلوب', 'Desired Property Type')}</p>
                <p className="font-bold text-md">{lead.property_type || t('غير محدد', 'Not specified')}</p>
              </div>
            </div>
            <div className="flex items-center gap-4 text-slate-700">
              <div className="bg-white p-2.5 rounded-xl shadow-sm border border-slate-100 text-emerald-600"><DollarSign size={18}/></div>
              <div>
                <p className="text-[10px] font-bold text-slate-400">{t('حجم الصفقة المتوقع', 'Expected Deal Size')}</p>
                <p className="font-black text-lg text-emerald-600">{Number(lead.expected_value).toLocaleString(numLocale)} {t('ج.م', 'EGP')}</p>
              </div>
            </div>
          </div>
          <UpdateLeadPanel
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 sm:p-8">
        <h3 className="text-xl font-black text-slate-800 flex items-center gap-2 mb-8 border-b border-slate-100 pb-4">
          ...
          {t('الصندوق الأسود (تاريخ التقارير)', 'Black Box (Report History)')}
        </h3>

        {(!reports || reports.length === 0) ? (
          <div className="text-center py-16 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
            <Clock size={40} className="mx-auto text-slate-300 mb-4" />
            <h4 className="text-lg font-black text-slate-700 mb-1">{t('لا يوجد سجلات سابقة', 'No previous records')}</h4>
            <p className="text-sm text-slate-500 font-medium">{t('لم يتم إضافة أي تقارير متابعة لهذا العميل حتى الآن من قبل فريق المبيعات.', 'No follow-up reports have been added for this client yet by the sales team.')}</p>
          </div>
        ) : (
                ...
                <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:border-blue-200 transition-all">
                  <div className="flex justify-between items-start mb-3 border-b border-slate-50 pb-3">
                    <span className="text-xs font-black text-slate-900 bg-slate-100 px-2 py-1 rounded-md">
                      {report.profiles?.full_name || t('وكيل مبيعات (FAST INVESTMENT)', 'Sales Agent (FAST INVESTMENT)')}
                    </span>
                    <time className="text-[11px] font-bold text-slate-400 bg-white border border-slate-100 px-2 py-1 rounded-md shadow-sm">
                      {new Date(report.created_at).toLocaleDateString(numLocale, { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </time>
                  </div>
