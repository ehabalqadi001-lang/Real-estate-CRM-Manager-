...
export default function UpdateLeadPanel({ leadId, currentStatus, currentTemp, currentValue }: Props) {
  ...
  return (
    <div className="mt-5 border-t border-slate-100 pt-5">
      ...
      >
        <Edit3 size={13} /> {t('تحديث بيانات العميل', 'Update Client Data')}
      ...
      {open && (
        <form onSubmit={handleSubmit} className="mt-4 space-y-3">
          <div className="grid grid-cols-1 xs:grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-bold text-slate-500 block mb-1">{t('الحالة', 'Status')}</label>
              <select name="status" defaultValue={currentStatus ?? 'Fresh Leads'}
                ...
                {STATUSES.map(s => <option key={s} value={s}>{t(STATUS_AR[s] ?? s, s)}</option>)}
              </select>
            </div>
            <div>
              <label className="text-[10px] font-bold text-slate-500 block mb-1">{t('الحرارة', 'Temperature')}</label>
              <select name="temperature" defaultValue={currentTemp ?? 'warm'}
                className="w-full border border-slate-200 rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-blue-400/30">
                <option value="hot">🔥 {t('ساخن', 'Hot')}</option>
                <option value="warm">📈 {t('دافئ', 'Warm')}</option>
                <option value="cold">❄️ {t('بارد', 'Cold')}</option>
              </select>
            </div>
          </div>
          <div>
            <label className="text-[10px] font-bold text-slate-500 block mb-1">{t('القيمة المتوقعة (ج.م)', 'Expected Value (EGP)')}</label>
            <input name="expected_value" type="number" min="0" defaultValue={currentValue ?? ''}
              className="w-full border border-slate-200 rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-blue-400/30" />
          </div>
          <div>
            <label className="text-[10px] font-bold text-slate-500 block mb-1">{t('ملاحظات', 'Notes')}</label>
          ...
          <button type="submit" disabled={pending}
            ...
            {saved ? <><CheckCircle size={12} /> {t('تم الحفظ', 'Saved')}</> : pending ? t('جاري الحفظ...', 'Saving...') : t('حفظ التحديثات', 'Save Updates')}
          </button>
        </form>
      )}
    </div>
