    ...
    <div className="min-h-screen bg-slate-50 p-4 sm:p-6" dir="rtl">
      <div className="mx-auto max-w-2xl space-y-5">
        <Link href="/dashboard/leads" className="inline-flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-slate-900">
          <ArrowRight size={16} /> {t('العودة للعملاء المحتملين', 'Back to Leads')}
        </Link>

        <div className="rounded-3xl border border-slate-100 bg-white p-4 sm:p-6 shadow-sm">
          <div className="mb-6 border-b border-slate-100 pb-5">
            <h1 className="text-2xl font-black text-slate-950">{t('إضافة عميل محتمل', 'Add New Lead')}</h1>
            <p className="mt-1 text-sm text-slate-500">{t('سجل بيانات العميل ليظهر داخل مسار المبيعات الخاص بك.', 'Register client data to appear in your sales pipeline.')}</p>
          </div>
          ...
          <NewLeadForm action={createLeadAction}>
            <div className="space-y-1.5">
              <label className="flex items-center gap-2 text-sm font-black text-slate-700">
                <User size={16} /> {t('اسم العميل', 'Client Name')}
              </label>
              <input
                name="clientName"
                required
                ...
                placeholder={t('مثال: أحمد محمود', 'e.g. John Smith')}
              />
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                <label className="flex items-center gap-2 text-sm font-black text-slate-700">
                  <Phone size={16} /> {t('رقم الهاتف', 'Phone Number')}
                </label>
                <input
                  name="phone"
                  type="tel"
              ...
              <div className="space-y-1.5">
                <label className="flex items-center gap-2 text-sm font-black text-slate-700">
                  <Mail size={16} /> {t('البريد الإلكتروني', 'Email')}
                </label>
                <input
                  name="email"
                  type="email"
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                <label className="flex items-center gap-2 text-sm font-black text-slate-700">
                  <Building2 size={16} /> {t('نوع العقار', 'Property Type')}
                </label>
                <select
                  name="propertyType"
              ...
              <div className="space-y-1.5">
                <label className="flex items-center gap-2 text-sm font-black text-slate-700">
                  <DollarSign size={16} /> {t('القيمة المتوقعة', 'Expected Value')}
                </label>
                <input
                  name="expectedValue"
                  type="number"
