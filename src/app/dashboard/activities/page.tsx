    ...
    <div className="p-8 space-y-8 min-h-screen bg-slate-50/50">
      ...
      <div className="flex justify-between items-center bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900">{t('مهامي اليومية والمواعيد', 'My Daily Tasks & Appointments')}</h1>
            <p className="text-sm font-bold text-slate-500 mt-1">{t('قائمة الاتصالات والاجتماعات المجدولة', 'Scheduled calls and meetings')}</p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="p-5 border-b border-slate-100 bg-amber-50 flex justify-between items-center">
            <h3 className="text-lg font-black text-amber-900 flex items-center gap-2">
              <Clock size={20} className="text-amber-600" /> {t('قيد الانتظار', 'Pending')} ({pendingActivities.length})
            </h3>
          </div>
          <div className="p-5 space-y-4">
            {pendingActivities.length === 0 ? (
              <p className="text-center text-slate-400 font-bold py-8">{t('لا توجد مهام معلقة! عمل رائع.', 'No pending tasks! Great work.')}</p>
            ) : (
                ...
                <div key={task.id} className="p-4 border border-slate-100 rounded-xl hover:border-blue-200 hover:shadow-md transition-all bg-white relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span className="text-[10px] font-bold text-amber-700 bg-amber-100 px-2 py-1 rounded-md">
                      {new Date(task.scheduled_at).toLocaleString(numLocale, { hour: '2-digit', minute:'2-digit', month: 'short', day: 'numeric' })}
                    </span>
                  </div>
                  <p className="text-sm text-slate-600 font-medium mb-4 bg-slate-50 p-2 rounded-lg">{task.notes || t('لا توجد ملاحظات إضافية.', 'No additional notes.')}</p>
                  <div className="flex justify-between items-center pt-3 border-t border-slate-50">
                    <Link href={`/dashboard/leads/${task.lead_id}`} className="text-xs font-black text-blue-600 hover:text-blue-800 flex items-center gap-1">
                      {t('ملف العميل', 'Client Profile')} <ArrowLeft size={14}/>
                    </Link>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden opacity-80 hover:opacity-100 transition-opacity">
          <div className="p-5 border-b border-slate-100 bg-emerald-50 flex justify-between items-center">
            <h3 className="text-lg font-black text-emerald-900 flex items-center gap-2">
              <CheckCircle2 size={20} className="text-emerald-600" /> {t('تم الإنجاز اليوم', 'Completed Today')} ({completedActivities.length})
            </h3>
          </div>
          <div className="p-5 space-y-4">
            {completedActivities.length === 0 ? (
              <p className="text-center text-slate-400 font-bold py-8">{t('سجل الإنجازات فارغ حتى الآن.', 'No completions yet today.')}</p>
            ) : (
                ...
                <div key={task.id} className="p-4 border border-emerald-100 rounded-xl bg-emerald-50/30 flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-bold text-emerald-600 mt-1">{t('أُنجزت:', 'Done:')} {new Date(task.done_at).toLocaleTimeString(numLocale, { hour: '2-digit', minute:'2-digit' })}</p>
                    </div>
                  </div>
