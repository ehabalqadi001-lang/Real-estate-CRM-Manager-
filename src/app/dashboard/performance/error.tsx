...
export default function PerformanceError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  const { t } = useI18n()
  useEffect(() => { console.error('Performance Error:', error) }, [error])
  return (
    <div className="flex h-[80vh] flex-col items-center justify-center p-4 sm:p-8 text-center" dir="rtl">
      <div className="bg-red-50 p-4 sm:p-8 rounded-3xl border border-red-100 max-w-lg w-full shadow-lg">
        ...
        <h2 className="text-2xl font-black text-slate-900 mb-3">{t('خطأ في صفحة الأداء', 'Performance Page Error')}</h2>
