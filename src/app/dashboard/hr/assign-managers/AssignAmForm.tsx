  ...
  return (
    <form action={action} className="flex flex-col gap-2">
      ...
      <div className="flex gap-2">
        ...
        >
          <option value="">{t('اختر Account Manager…', 'Select Account Manager…')}</option>
          {amOptions.map((am) => (
        ...
        >
          {isPending ? '…' : currentAmId ? t('تغيير', 'Change') : t('تعيين', 'Assign')}
        </button>
      </div>
      {state.message && (
...
export function RemoveAmForm({ brokerProfileId }: { brokerProfileId: string }) {
  ...
  return (
    <form action={action}>
      ...
      >
        {isPending ? '…' : t('إلغاء التعيين', 'Remove Assignment')}
      </button>
