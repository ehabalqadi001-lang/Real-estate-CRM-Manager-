  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-sm font-black text-[var(--fi-emerald)]">Account Management</p>
          <h1 className="mt-1 text-xl sm:text-3xl font-black text-[var(--fi-ink)]">مساحة عمل مدير الحسابات</h1>
      ...
      <div className="overflow-hidden rounded-xl border border-[var(--fi-line)] bg-[var(--fi-paper)] shadow-sm">
        <div className="flex items-center justify-between border-b border-[var(--fi-line)] px-5 py-4">
          <p className="font-black text-[var(--fi-ink)]">قائمة الحسابات</p>
          <Badge className="bg-[var(--fi-soft)] text-[var(--fi-emerald)]">{users.length} حساب</Badge>
        </div>

        {users.length === 0 ? (
          <div className="p-10 text-center">
            <Users className="mx-auto mb-3 size-10 text-[var(--fi-line)]" />
            <p className="font-black text-[var(--fi-ink)]">لا توجد حسابات بعد</p>
          </div>
        ) : (
      ...
      <div className="rounded-xl border border-[var(--fi-line)] bg-[var(--fi-paper)] p-5 shadow-sm">
        <p className="mb-4 flex items-center gap-2 font-black text-[var(--fi-ink)]">
...
function Metric({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-xl border border-[var(--fi-line)] bg-[var(--fi-paper)] p-4 shadow-sm">
      <div className="flex items-center gap-2 text-[var(--fi-emerald)]">{icon}</div>
...
function AccountRow({ user }: { user: Profile }) {
  const statusColors: Record<string, string> = {
    active: 'bg-[var(--fi-soft)] text-[var(--fi-emerald)]',
  ...
  return (
    <div className="grid items-center gap-3 px-5 py-4 md:grid-cols-[1fr_140px_120px_160px_auto]">
      <div>
        <p className="font-black text-[var(--fi-ink)]">{user.full_name ?? 'بدون اسم'}</p>
        <p className="mt-0.5 flex items-center gap-1 text-xs font-semibold text-[var(--fi-muted)]">
      ...
      <span className="text-xs font-semibold capitalize text-[var(--fi-muted)]">{user.account_type ?? user.role ?? '—'}</span>
      <div className="flex items-center gap-1 text-xs font-semibold text-[var(--fi-muted)]">
        <Star className="size-3 text-[#C9964A]" />
        0 نقطة
      </div>
      <div className="flex gap-1.5">
        <Button size="sm" variant="outline" className="h-7 border-[var(--fi-line)] px-2">
