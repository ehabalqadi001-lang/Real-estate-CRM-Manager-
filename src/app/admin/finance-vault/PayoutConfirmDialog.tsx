...
export function PayoutConfirmDialog({ payoutId, recipientName, amount }: Props) {
  ...
  if (step === 'done') {
    return (
      <span className="flex items-center gap-1.5 text-xs font-semibold text-[var(--fi-emerald)]">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-sm rounded-2xl border border-[var(--fi-line)] bg-[var(--fi-paper)] p-4 sm:p-6 shadow-2xl">
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-semibold text-[var(--fi-muted)]">خطوة {step === 'confirm' ? '1' : '2'} من 2</p>
          </div>
        </div>

        {step === 'confirm' && (
          <>
            <div className="mb-5 rounded-lg bg-[var(--fi-soft)] p-4">
              <p className="text-xs font-semibold text-[var(--fi-muted)]">المستلم</p>
              <p className="mt-1 font-black text-[var(--fi-ink)]">{recipientName}</p>
              <p className="mt-2 text-xs font-semibold text-[var(--fi-muted)]">المبلغ</p>
              <p className="mt-1 text-xl font-black text-[#C9964A]">{amount}</p>
            </div>
            <div className="flex gap-2">
              <Button
                variant="outline"
                className="flex-1 border-[var(--fi-line)]"
        ...
        {step === 'pin' && (
          <>
            ...
            <div className="mt-4 flex gap-2">
              ...
              <Button
                disabled={pending}
                className="flex-1 fi-primary-button font-semibold"
