...
export default function MortgageCalculator() {
  ...
  const calcData = useMemo(() => {
    ...
    for (let i = 1; i <= totalPeriods; i++) {
      ...
      schedule.push({
        paymentNumber: i,
        date: currentDate.toLocaleDateString(numLocale, { year: 'numeric', month: 'long' }),
  ...
  }, [propertyPrice, downPaymentPct, years, interestRate, frequency, numLocale])

  const handlePrint = () => {
    window.print()
  }

  const formatCurrency = (val: number) => new Intl.NumberFormat(numLocale, { maximumFractionDigits: 0 }).format(val)
    ...
    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      <div className="bg-slate-950 p-8 text-white grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800">
          <p className="text-slate-400 text-sm font-bold mb-2">{t('قيمة القسط (دوري)', 'Installment Amount (Periodic)')}</p>
          <h3 className="text-3xl font-black text-blue-400">{formatCurrency(calcData.paymentAmount)} <span className="text-lg">{currency}</span></h3>
        </div>
        <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800">
          <p className="text-slate-400 text-sm font-bold mb-2">{t('المبلغ الممول (المتبقي)', 'Financed Amount (Remaining)')}</p>
          <h3 className="text-3xl font-black text-white">{formatCurrency(calcData.financedAmount)} <span className="text-lg">{currency}</span></h3>
        </div>
        <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800">
          <p className="text-slate-400 text-sm font-bold mb-2">{t('إجمالي الفوائد البنكية', 'Total Bank Interest')}</p>
          <h3 className="text-3xl font-black text-emerald-400">{formatCurrency(calcData.totalInterest)} <span className="text-lg">{currency}</span></h3>
        </div>
      </div>
      ...
      <div className="p-8 grid grid-cols-1 xl:grid-cols-3 gap-10">
        ...
        <div className="xl:col-span-1 space-y-6 bg-slate-50 p-6 rounded-2xl border border-slate-100 print:hidden">
          <h3 className="font-bold text-slate-800 border-b border-slate-200 pb-3 mb-4">{t('محددات الخطة السعرية', 'Pricing Plan Settings')}</h3>

          <div>
            <label className="flex items-center gap-2 text-xs font-bold text-slate-700 mb-2"><DollarSign size={14}/> {t('إجمالي قيمة العقار', 'Total Property Value')}</label>
          ...
          <div>
            <label className="flex justify-between items-center text-xs font-bold text-slate-700 mb-2">
              <span className="flex items-center gap-2"><ArrowDownToLine size={14}/> {t('الدفعة المقدمة (Down Payment)', 'Down Payment')}</span>
              <span className="text-blue-600 bg-blue-50 px-2 py-0.5 rounded">{formatCurrency(calcData.downPaymentValue)} {currency}</span>
            </label>
          ...
          <div>
            <label className="flex items-center gap-2 text-xs font-bold text-slate-700 mb-2"><Calendar size={14}/> {t('سنوات التقسيط', 'Installment Years')}</label>
          ...
          <div>
            <label className="flex items-center gap-2 text-xs font-bold text-slate-700 mb-2"><Calendar size={14}/> {t('دورية السداد', 'Payment Frequency')}</label>
            <select value={frequency} onChange={(e) => setFrequency(Number(e.target.value))} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 font-bold bg-white">
              <option value={12}>{t('شهري', 'Monthly')}</option>
              <option value={4}>{t('ربع سنوي', 'Quarterly')}</option>
              <option value={2}>{t('نصف سنوي', 'Semi-Annually')}</option>
              <option value={1}>{t('سنوي', 'Annually')}</option>
            </select>
          </div>

          <div>
            <label className="flex items-center gap-2 text-xs font-bold text-slate-700 mb-2"><Percent size={14}/> {t('نسبة الفائدة السنوية (0 = خطة مطور)', 'Annual Interest Rate (0 = Developer Plan)')}</label>
        ...
        <div className="xl:col-span-2 flex flex-col h-[700px]">
          <div className="flex justify-between items-center mb-6">
            <div>
               <h3 className="text-xl font-bold text-slate-900">{t('جدول خطة السداد المقترحة', 'Proposed Payment Schedule')}</h3>
               <p className="text-sm text-slate-500 mt-1">{t('تفاصيل الدفعات والأرصدة المتبقية', 'Payment details and remaining balances')}</p>
            </div>
            <button onClick={handlePrint} className="flex items-center gap-2 bg-white border-2 border-slate-200 hover:border-slate-800 text-slate-800 px-4 py-2 rounded-xl transition-all font-bold text-sm print:hidden shadow-sm">
              <Printer size={16} /> {t('طباعة العرض', 'Print Offer')}
            </button>
          </div>
            ...
            <div className="overflow-y-auto custom-scrollbar flex-1">
              <table className="w-full text-sm text-right">
                <thead className="bg-slate-50 sticky top-0 z-10 border-b border-slate-200 shadow-sm">
                  <tr>
                    <th className="px-6 py-4 font-bold text-slate-600">{t('# القسط', '# Installment')}</th>
                    <th className="px-6 py-4 font-bold text-slate-600">{t('تاريخ الاستحقاق (تقريبي)', 'Due Date (Approx.)')}</th>
                    <th className="px-6 py-4 font-bold text-slate-600">{t('قيمة القسط', 'Installment Amount')}</th>
                    <th className="px-6 py-4 font-bold text-slate-600">{t('الرصيد المتبقي', 'Remaining Balance')}</th>
                  </tr>
                </thead>
